...
import com.jawnz.gate.config.KafkaProperties;
...
class JawnzgatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        JawnzgatewayKafkaResource kafkaResource = new JawnzgatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jawnzgateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jawnzgateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
